...
const OutfitSuggestion = () => {
  ...
  const generateOutfitReason = () => {
    ...
    const reasons = [`This outfit complements your ${userProfile.skinTone} skin tone`, `The colors work well with your ${userProfile.hairColor} hair`, `The color palette enhances your ${userProfile.eyeColor} eyes`];
    ...
    if (currentOutfit.score) {
      const scoreText = currentOutfit.score >= 85 ? "an exceptional match" : currentOutfit.score >= 70 ? "a great match" : "a good match";
      reasons.push(`Our AI thinks this is ${scoreText} for you`);
    }

    // Join reasons with commas and "and" for the last one
    const reasonsText = reasons.length > 1 ? `${reasons.slice(0, -1).join(", ")} and ${reasons[reasons.length - 1]}` : reasons[0];
  ...
  return <div className="pb-20 animate-fade-in">
      ...
      <OccasionFilters currentOccasion={currentOccasion} onOccasionChange={handleOccasionChange} />
      
      <UserProfileCard selfieUrl={userProfile.selfieUrl} skinTone={userProfile.skinTone} hairColor={userProfile.hairColor} eyeColor={userProfile.eyeColor} />
      
      <OutfitDisplay outfit={currentOutfit} outfitReason={outfitReason || `This outfit has been selected for ${capitalizeWords(currentOccasion)} occasions based on your personal style and today's weather.`} onRefresh={handleRefresh} onWear={handleWear} />
      
      <FeedbackDialog open={feedbackOpen} onOpenChange={setFeedbackOpen} onSubmit={handleFeedbackSubmit} />
    </div>;
};
